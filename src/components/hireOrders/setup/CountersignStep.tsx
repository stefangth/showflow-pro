...
export function CountersignStep({ orgId, onDone }: { orgId: string | null; onDone: () => void }) {
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t("countersignStep.saved"));
      onDone();
    },
  ...
  if (stored.isError) {
    return (
      <Alert variant="destructive">
        <AlertDescription>
          {t("countersignStep.loadError", { message: (stored.error as Error).message })}
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">{t("countersignStep.intro")}</p>
