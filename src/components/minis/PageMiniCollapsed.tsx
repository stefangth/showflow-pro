...
export function PageMiniCollapsed({ label, hint, ctaLabel, onOpen }: PageMiniCollapsedProps) {
  return (
    <div className="flex items-center gap-2.5 rounded-card border-[0.5px] border-border bg-card px-3.5 py-2.5">
      ...
      <button onClick={onOpen} className="rounded-card border-[0.5px] border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-hover-tint">{ctaLabel}</button>
    </div>
  );
}
