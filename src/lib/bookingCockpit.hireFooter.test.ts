...
describe("computeHireFooter", () => {
  it("is ready only when the DB status is fully_filled", () => {
    const f = computeHireFooter({ status: "fully_filled", slots, confirmedMain: 4, confirmedUnderstudy: 2 });
    expect(f.ready).toBe(true);
    expect(f.badgeLabel).toBe("READY");
    expect(f.detail).toMatch(/All slots confirmed/);
  });
  ...
  it("does NOT green-light drafting when one role is short, even if the flat sum hits total", () => {
    ...
    const f = computeHireFooter({ status: "partially_filled", slots, confirmedMain: 6, confirmedUnderstudy: 0 });
    ...
    expect(f.detail).toBe("Waiting on 2 of 6 slots");
  });

  it("counts remaining per role, not as a flat total", () => {
    const f = computeHireFooter({ status: "partially_filled", slots, confirmedMain: 1, confirmedUnderstudy: 2 });
    expect(f.remaining).toBe(3); // 3 main short + 0 understudy short
  });

  it("no slot config → zero remaining, not ready", () => {
    const f = computeHireFooter({ status: "partially_filled", slots: null, confirmedMain: 0, confirmedUnderstudy: 0 });
    expect(f.ready).toBe(false);
