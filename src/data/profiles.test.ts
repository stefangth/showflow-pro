...
describe("password methods", () => {
  ...
  it("changes a password with the current password", async () => {
    const fake = createFakeSupabase({ "auth:updateUser": { data: { user: { id: "u1" } }, error: null } });
    await changeMyPassword(fake as never, { password: "new-secret", currentPassword: "old-secret" });
    expect(fake.calls).toContainEqual({ table: "auth", method: "updateUser", args: [{ password: "new-secret", currentPassword: "old-secret" }] });
  });
