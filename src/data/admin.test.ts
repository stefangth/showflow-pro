...
describe("fetchAdminAuditLogs", () => {
  it("selects newest-first with the artist join and applies the limit", async () => {
    ...
    const res = await fetchAdminAuditLogs(fake as never, 50);
  ...
  it("returns [] when data is null", async () => {
    ...
    expect(await fetchAdminAuditLogs(fake as never, 50)).toEqual([]);
  });

  it("throws on error", async () => {
    const fake = createFakeSupabase({ booking_audit_log: { data: null, error: { message: "boom" } } });
    await expect(fetchAdminAuditLogs(fake as never, 50)).rejects.toBeTruthy();
  });
});
...
describe("fetchAdminSyncLogs", () => {
  it("selects newest-first by synced_at with the limit", async () => {
    ...
    const res = await fetchAdminSyncLogs(fake as never, 20);
    expect(res).toEqual(rows);
  ...
  it("throws on error", async () => {
    ...
    await expect(fetchAdminSyncLogs(fake as never, 20)).rejects.toBeTruthy();
  });
});
