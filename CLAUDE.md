...
- **`show_dates.status` is DB-computed.** A Postgres trigger (`sync_show_date_status_trigger` on `bookings`) automatically sets status to `open | partially_filled | fully_filled` based on confirmed booking count vs `slots_per_date`. Only `cancelled` is set by mutations directly. Do not set status manually in client code.
