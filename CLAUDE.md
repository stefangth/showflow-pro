...
Run the CI stack locally with the two-tier convention: **`npm run verify:fast`** (Docker-free inner loop — lint, typecheck, build, unit+coverage, Deno) and **`npm run verify:full`** (adds pgTAP + Playwright e2e; needs the local stack). A pre-push hook (`core.hooksPath` → `.githooks/`, installed by `npm ci`'s `prepare` script) requires `verify:fast` before every push, but reuses a successful fast/full result when the exact source snapshot is unchanged; secret scanning still runs on every push. Bypass with `git push --no-verify` or `SHOWFLOW_SKIP_VERIFY=1` if you deliberately need to push past a known-broken check. Full runbook: [`docs/runbooks/local-development-stack.md`](docs/runbooks/local-development-stack.md).
