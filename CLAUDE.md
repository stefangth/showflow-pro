...
| `scripts/sync-mirrors.mjs` | Generator for every dual-homed frontend/edge file listed in `scripts/mirrors.manifest.json` (currently 4 pairs: `src/lib/hireOrders/pdfCopy.ts`, `src/integrations/supabase/types.ts`, and the sentinel-delimited registry blocks in `src/lib/capabilities.ts` and `src/lib/entitlements.ts`). `npm run sync:mirrors` regenerates every target from its source; `npm run sync:mirrors:check` fails CI on drift. Never hand-edit a generated target or block. `src/lib/hireOrders/types.ts` (structural, five files combined into one) and the hire-order consent text (`SignHireOrderDialog.tsx` / `generate-hire-orders/index.ts`) are dual-homed too but are deliberately NOT in the manifest and stay hand-maintained |
