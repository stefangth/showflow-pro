...
src/
  ...
  config/
    app.config.ts  # Feature flags (FEATURES), route constants (ROUTES), BOOKING_ENGINE_DEFAULTS,
                   #   CHAT_ARCHIVE_DAYS, APP_META (NAME/DESCRIPTION/VERSION/MARKETING_URL), ROLES
  data/            # Data-access layer: fetchX(client, args) / mutateX(client, args) functions
                   ...
                   #   Domains: artists, bookings, cities, customFields, invitations, members,
                   #   notifications, orgs, platform, profiles, settings, skills.
                   #   Airtable sub-modules: airtableMapping, airtableSchema, airtableSync.
  ...
  lib/             # Shared utilities: utils.ts (cn helper), dates.ts (parseDateOnly,
                   ...
                   #   avatar.ts, bookings.ts,
                   #   customFields.ts (CustomFilterState, slugifyKey, airtableTypeToCustomType),
                   #   identity.ts (re-exports resolveContactEmail/resolveAccountDisplayName from _shared),
                   #   settings.ts (showSlots, computeSchedulingWarnings)
...
- **`show_dates.status` is DB-computed.** A Postgres trigger (`sync_show_date_status_trigger` on `bookings`) automatically sets status to `open | partially_filled | fully_filled` based on confirmed booking counts vs the `main_cast_slots` + `understudy_slots` columns on `shows`. `cancelled` is set either by direct mutation or by `airtable-poll` when the configured `status_field` value equals the org's `cancelled_value` mapping. Do not set status manually in client code. A trigger on `shows` recomputes that show's dates when its `program`, `sub_program`, `main_cast_slots`, or `understudy_slots` change.
...
- **Airtable sync is org-aware.** The `airtable-poll` edge function upserts `show_dates` from Airtable and is scheduled via pg_cron (`*/5 * * * *`). Enable per-org by setting `airtable_sync_enabled = true` in `app_settings` for that org (via Settings → Airtable) and configuring `airtable_base_id`, `airtable_table_name`. The Airtable API key is stored per-org in **Supabase Vault** (not a global edge function secret) — see the `get_org_airtable_key` / `set_org_airtable_key` RPCs and the `20260604131000_org_airtable_vault.sql` migration. Catalog linking (Phase 2b) connects Airtable controlled-option values to catalog rows via `shows.airtable_program_key` and `cities.airtable_city_key` — grain-agnostic `text`, **unique per org** (partial index where not null). The link grain (a single sub-program option vs a Program+Sub-Programm composite) is chosen by the admin in the mapping UI, not fixed in the DB (2b-UI links **sub-program-only**; the composite path is deferred). The Settings → Airtable Sync tab (`src/components/settings/AirtableSyncTab.tsx`) drives schema-based base/table/field selection (dropdowns from `airtable-schema`, typed fallback when the PAT lacks `schema.bases:read`), writes `app_settings.airtable_field_map`, and does catalog linking (import-all + unlink). City link keys are **case-insensitive** (`buildCityKey` = trim+lowercase via the shared `normalizeCityName`), so **"Import all" auto-links** an Airtable option to an existing same-name city instead of creating a duplicate, and each unlinked option has a per-row "link to existing city" control. The field map (`AirtableFieldMap` in `src/data/airtableMapping.ts`) also supports three optional cancellation fields: `status_field` (Airtable field name), `cancelled_value` (the option string that means cancelled), and `cancellation_reason_field` (free-text reason). When a record matches, `airtable-poll` sets `show_dates.status = 'cancelled'`. Session-time changes (add/remove/retime) and cancellations are recorded in the `show_date_change_log` table and coalesced by `_shared/scheduleChanges.ts` for inclusion in the confirmation digest. The dead "Filter Mappings" Settings card was removed.
- **Custom fields are Airtable-sourced.** The `custom_field_definitions` table (org-scoped) stores admin-configured extra columns synced from Airtable fields (via the Settings → Airtable Sync tab). Each definition has `entity` (e.g. `'show_date'`), `key`, `label`, `type` (text/number/date/boolean/select), `source_field` (Airtable field name), optional `options`, and filterable/sortable flags. The data layer lives in `src/data/customFields.ts`; type helpers in `src/lib/customFields.ts` (`CustomFilterState`, `slugifyKey`, `airtableTypeToCustomType`). `CustomFieldFilter` (`src/components/filters/`) provides a type-switched filter control for each filterable definition.
- **Schedule-change notifications are DB-driven.** The `show_date_change_log` table records session-time transitions (`session_added`, `session_removed`, `session_retimed`) and cancellations triggered by `airtable-poll`. `_shared/scheduleChanges.ts` (pure, no I/O) coalesces multiple log rows into one net summary per date — a reverted change cancels out, cancellation beats session noise. The confirmation digest reads these rows and includes a schedule-change summary section in the email.
- **Offer tier management.** Producers and admins can open and close offer tiers from the Offers card in `ShowDateDetailSheet`. Opening calls `open-offer-tier` (existing). Closing calls `close-offer-tier`, which stamps `show_date_offer_tiers.closed_at` (stops escalation + at-risk alerts). With `withdraw=true`, it also cancels still-`suggested` bookings (`cancellation_reason='tier_closed'`); `soft_booked`/`confirmed` bookings are never touched. Re-opening a closed tier clears `closed_at`.
...
- One folder per function under `supabase/functions/<name>/index.ts`. Current categories:
  ...
  - **Booking engine:** `open-offer-tier` (create suggested bookings), `close-offer-tier` (stamp `show_date_offer_tiers.closed_at`; with `withdraw=true` also cancel still-`suggested` bookings with `cancellation_reason='tier_closed'`), `expire-offers` (hourly expiry), `send-offer-digest` (daily 19:00 Berlin), `send-confirmation-digest` (daily 20:00 Berlin; includes schedule-change summary from `show_date_change_log`). All cron functions are org-aware: they iterate active orgs via `getActiveOrgs(admin)` from `_shared/settings.ts` and resolve settings per-org with `resolveOrgSetting`.
...
| `src/data/platform.ts` | Super-admin data access: `fetchAllOrgs`, `provisionOrg`, `fetchPlatformOrgStats`, `fetchPlatformBookingDefaults` / `savePlatformBookingDefaults`, platform admin CRUD |
