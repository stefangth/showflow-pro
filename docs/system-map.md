...
| `handle-email-suppression` | Resend webhook | HMAC signature (Standard Webhooks) | `suppressed_emails`, `email_send_log` | none |
...
Inbound: Resend bounces/complaints → `handle-email-suppression` (HMAC-verified) → `suppressed_emails`; unsubscribe links → `handle-email-unsubscribe` (atomic token consumption) → `suppressed_emails`.

## 8. Observability — who watches the watchers

Every cron dispatch is recorded (`cron_health_dispatch` + pg_net's `net._http_response`); `cron-health-watcher` joins them every 15 min via `cron_health_scan()`, classifies per-job against max-silence windows (`airtable-poll` 30 min, hourly jobs 130 min, digests 1320 min), and alerts super-admins only on the transition into `failing`/`stale` — recovery is in-app-only to avoid flap storms. The System Health tab (super-admin) reads `get_cron_health()` for the cron dashboard and `platform-edge-metrics` for per-function invocation/error/latency stats from the Supabase Analytics API. Every email attempt — sent, failed, suppressed, pref-blocked — lands in `email_send_log`. Known blind spot: the watcher cannot observe its own liveness (its own dispatch row is always in-flight when it runs); check `cron.job_run_details` externally if the watcher itself goes quiet. Cites: `cron-health-watcher/index.ts:21-163`, `20260623042017_cron_dispatch_capture.sql`.
...
- **Trigger:** Resend webhook (bounce/complaint)
- **Auth:** Standard-Webhooks HMAC, constant-time, 300 s staleness window (`index.ts:7-48,96`); `verify_jwt = false`
- **Writes:** `suppressed_emails` upsert (reason bounce|complaint), `email_send_log` status row
- **Failure:** bad signature → 401; non-suppression events acknowledged `{ignored:true}`; idempotent upsert
