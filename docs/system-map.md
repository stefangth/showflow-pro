...
- **Inputs:** `{action, org_id, ...}` — `draft` (`show_date_id`, optional `booking_ids`/`notify`), `issue` (`order_ids`), `preview` (`order_id`), `download-url` (`order_id`), `countersign-test` (no other fields — the Documenso instance URL is never client-supplied, see Effects)
