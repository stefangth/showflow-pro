...
| `expire-offers` | cron hourly + UI | cron secret ∨ admin/producer (reminder pass + escalation scan additionally limited to orgs entitled to the `booking_flow` module, via `filterEntitledOrgs`) | `bookings` (`reminder_sent_at`); `notifications` (`offer_expiring`, `tier_escalated`, `cast_escalation_requested`), `show_date_offer_tiers` (escalation stamp); `expire_soft_bookings()` RPC → `bookings` | `offer-expiry-reminder` email 24 h before expiry (gated `expiry_reminder`); `auto_escalate` on: auto-opens the next tier of the same effective ladder, no email; else `cast-escalation-requested` email to producers |
