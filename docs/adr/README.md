...
- **Feature entitlements (paid modules) are per-org, not compile-time flags.** The old global `FEATURES` object in `app.config.ts` was dead code and has been removed. The registry of modules lives in `src/lib/entitlements.ts`, mirrored byte-for-byte in `supabase/functions/_shared/entitlements.ts`, with a third home in the SQL `CASE` inside `is_feature_enabled(_org, _feature)` — all three carry the same defaults and must change in the same PR. A module's state per org is a row in `org_entitlements` (super-admin-write-only; no row = registry default), toggled from the Platform console (Organizations → Edit org → Modules). Check entitlement with `useFeature('<key>')` in components, `checkFeature`/`requireFeature` in edge functions, and `is_feature_enabled` in triggers and RLS. Gate whole feature blocks, never individual lines, and prefer degrading to the classic default over breaking: `booking_flow` resolution falls back to `normalizeBookingFlow(null)` when unentitled rather than erroring.
- **Per-org capabilities ("user group rights") gate WHO may act; entitlements gate WHETHER a feature exists.** The registry of 27 producer capabilities lives in `src/lib/capabilities.ts` (`CAPABILITY_DEFS`: `key`, `action`, `role`, `group`, `risk`, `defaultEnabled`, optional `module`), mirrored byte-for-byte in `supabase/functions/_shared/capabilities.ts`, with a third home in the SQL `CASE` inside `capability_default(_capability)` — all three carry the same defaults and must change in the same PR (guarded by `src/lib/capabilitiesMirror.test.ts` and `capabilityDefaultsSql.test.ts`). Resolution is **layered**: platform lock → org override (`org_capabilities`) → platform default (`org_capability_policies`) → registry default, implemented once in `is_capability_enabled(_org, _capability)` and mirrored by `resolveCapability` in TS. Admins are NEVER a stored capability (always full), which structurally prevents a last-admin lockout. Org admins tune unlocked rights in **Settings → Roles & permissions**; platform admins set per-org defaults and may LOCK a right. Enforcement is defense in depth: `useCan(action)` in the UI (render read-only, never hide the page), `requireCapability` in edge functions, an in-body check in RLS-bypassing `SECURITY DEFINER` RPCs (`rename_org`, `bulk_import_artists`), and a capability-aware predicate on WRITE RLS policies. **Capabilities gate mutations only** — never add one to a SELECT policy; producer read is unconditional (the "read-only floor"). Where RLS granularity (role x table x command) cannot separate two UI actions, the finer one is UI-gated and server-covered by a coarser sibling (archive/reorder productions and edit_scheduling all resolve to `shows` UPDATE, covered by `manage_productions`); `app_settings` writes resolve per key through `app_setting_capability(key)`, where an unmapped key stays admin-only (fail-safe).
