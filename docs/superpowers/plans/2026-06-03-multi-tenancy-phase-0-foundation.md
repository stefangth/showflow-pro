...
5. **Isolation via a RESTRICTIVE policy, not a full policy rewrite** (replaces Task 0.3's drop-and-regenerate). Reading the existing tests showed `blocked_dates`/`bookings` enforce *per-artist* read isolation the test author calls "the critical guard" — a uniform "any member can read" rule would regress it. Instead, migration 3 adds ONE `AS RESTRICTIVE` policy per table (`org_isolation`: `is_org_member(org_id)` for `USING` + `WITH CHECK`), which `AND`s org-membership onto every existing permissive policy — preserving all intra-org semantics while making cross-org rows unreachable. Combined with a **bootstrap-org fallback** in `is_org_member` (any global-role holder is a member of the legacy bootstrap org), the existing pgTAP suite needs **zero edits**. The `has_role → has_org_role` per-org role-gating swap (and removing the fallback + `user_roles`) is Phase 1. `is_chat_participant` likewise stays as-is this phase — the restrictive policy supplies the org boundary on top of it.
