...
- [x] **`onboarding` namespace (shared)** — DONE (2026-08-15, own PR). Threaded a namespace-bound `TFunction<'onboarding'>` through the dashboard first-run + setup-rail copy MODULES, reading every string from the shared `onboarding` catalog (327 keys/language; EN byte-identical, DE authored dark). Migrated `stageChain.composeStageChain`; `firstRun` (welcome/railHeader/collapsed copy + adminTeamStep/injectAdminTeamStep); `moduleOnboarding` (static registries became `t`-driven builders `buildBookingOnboarding`/`buildHireOrderOnboarding`/`buildModuleOnboarding`/`buildArtistOnboarding` + `teamStepMeta`/`viewAsArtistTip`/`producerRoleNote`/`roleExplainerLinkLabel`/`producerRoleRuleTitle`; `ROLE_EXPLAINER_LINK_ROUTE`/`TEAM_STEP_KEY`/`ARTIST_STEP_KEYS` stay constants); `setupStatus.STEP_TITLES` → `stepTitles(t)`; `coverageCopy`. Wired LadderStep, EligibilityStep, TeamStep, ShowsStep, BookingSetupRail, BookingProducerWaitingCard, useModuleOnboardingRail, useDashboardFirstRun; tests rebind `i18n.getFixedT('en','onboarding')`. **Deferred (in PR body):** (1) `scheduleChangeCopy.ts` (consumed by `actionCopy.cancelBookingCopy`, which PR #289 moves into `bookingCopy` — localize it there to avoid a namespace collision); (2) `SAMPLE_PREVIEW` (static demo fixture); (3) standalone setup-component INLINE copy that never lived in these modules (LadderStep "No casts ranked.", the rail "Set up · N of M" header, etc.) — a separate component-copy pass.
