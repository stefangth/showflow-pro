...
7. **Small polish batch** (one commit): stale top-of-file JSDoc in both digest edge functions (they do not mention the flow gates); `d_bookings.detail.Cite` in `src/data/systemMap.ts` missing `20260714104826`; error log on the reminder pass's in-app notification insert in `expire-offers`; `booking-times` query in `ShowDateDetailSheet` lacks an `enabled` guard.
