...
- **Styling / tokens (verified 2026-08-17):** use a semantic Tailwind utility where one exists, else the CSS-var arbitrary form `*-[var(--token)]` (never a raw hex). Verified: `#6E5CF6`→`bg-primary`/`text-primary` — but ONLY for filled ticks, primary buttons, active-nav bg, brand mark; **accent TEXT `#5848D8`→`text-accent-600`** (eyebrows, action links, labels like "On"/"N left, blocking"/"Review"/"Change"/"Rank here") — do NOT use `text-primary` for accent text; active-nav text `#4738B0`→`text-accent-700`; `#F6F4EF`→`bg-background`; `#FFFFFF`→`bg-card`; `#FAF8F4`→`bg-muted`; `#EFEDE7`→`bg-[var(--surface-3)]`; `#5B5A57`→`text-muted-foreground`; `#8B8A85`→`text-[var(--text-faint)]`; `#E6E3E0`→`border-border`. Accent stops `bg-accent-50..900` do NOT take opacity modifiers (silently solid). Shadows → `shadow-elev1..4`, never `shadow-[var(--shadow-2)]`.
