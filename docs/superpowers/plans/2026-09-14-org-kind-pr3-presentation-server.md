...
- [ ] **Step 3: Implement.** In `registry.ts` (which lives outside the copy-map mirror, so it may import `VOCABULARY` and `type OrgKind` from `../orgKind.ts`), add `kind?: OrgKind` to `TemplatePresentationOptions`, and change the `resolveEmailCopy(options.copyOverride, options.locale)` call to pass the resolved table: `resolveEmailCopy(options.copyOverride, options.locale, VOCABULARY[options.kind ?? 'production'][options.locale])`. In `send-transactional-email/index.ts`, import `resolveOrgKind` from `../_shared/orgKind.ts`, and right after `const locale = await resolveOrgLocale(admin, orgId, localeOverride)` add `const kind = await resolveOrgKind(admin, orgId)`, then pass `kind` into the `resolveTemplatePresentation(...)` options object. Do not add an entitlement gate: workspace type is not a paid module.
