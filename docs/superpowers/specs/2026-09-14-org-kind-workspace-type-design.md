...
- **Editor previews.** The Email Templates editor shipped the workspace-type preview toggle (defaults to the org kind, threads `kind` through `EmailPreviewPane` to `preview-transactional-email`). The hire-order PDF template editor's live preview was also made kind-aware (resolves copy with the org's kind vocabulary) so a staffing org's preview matches the issued PDF; both were preview-vs-output mismatches caught in review.
...
- **Deferred follow-up.** `send-transactional-email`/`generate-hire-orders` read `org_language` and `org_kind` in separate sequential queries; combining or parallelizing them is a non-blocking optimization left for its own change.
