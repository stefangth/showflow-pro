...
| `countersigned` | The artist has countersigned: offline (manual mode, a producer marks it) or in-app (electronic mode, the artist signs on the order page). |
...
**Countersign: manual by default, electronic optional.** Settings > Hire orders lets an org pick one of two countersign modes. The default `manual` mode carries a countersignature block on the issued PDF that the artist signs offline; a producer then marks the order `countersigned` in the viewer. The `electronic` mode moves signing in-app: once an order is `issued`, the linked artist opens it on the order detail page, reviews the document shown there, and signs by typing their name or drawing a signature, after confirming a consent statement. Signing re-renders the PDF with a signature block plus an appended signature certificate page (method, signer, timestamp, IP, consent text, and a hash of the original issued document), stores that signed PDF and a `hire_order_signatures` audit row, and flips the order straight to `countersigned`. The artist gets the signed PDF by email and producers do too when the org turns on the "also email producers the signed copy" setting; the order's assigned producers (falling back to org admins when none resolve) and the signing artist each get an in-app notification. This is a single-signer, artist-initiated flow: there is no producer-initiated signing, no multi-party routing, and no reminder/expiry on an unsigned electronic order.

**Documenso (not in use).** An earlier `documenso` countersign mode integrated with a self-hosted or hosted Documenso instance for e-signature: issuing would send the PDF to Documenso via `_shared/documenso.ts`, and the `documenso-webhook` edge function would flip the order to `countersigned` when Documenso reported the envelope completed. In-app electronic signing superseded this before any org went live with it, so the settings UI no longer offers `documenso` as a selectable mode. The code (`_shared/documenso.ts`, `documenso-webhook`) stays deployed but dormant, in case a future org needs a self-hosted Documenso integration instead of (or alongside) in-app signing.
