...
async function syncOrg(deps: Deps, orgId: string, baseId: string, tableName: string, apiKey: string, fieldMap: FieldMap, viewName: string): Promise<OrgSyncResult> {
  ...
  // module: while off, none of the gated engine paths run (auto-open, escalation, reminders,
  // digests, at-risk) and manual open-offer-tier is refused, so nothing here opens a tier
  // either; the one thing still ticking is offer-expiry cancellation, which gates on the
  // module entitlement (expire_soft_bookings), not this JSONB switch), the booking_flow
  ...
  // resolveBookingFlow itself fails open to permissive defaults, including active: true
  // and auto_open_tier1: true, on an entitlement-check failure, so flow.active/
  // flow.auto_open_tier1 alone are NOT a safe gate for an unentitled org), auto_open_tier1
  // (owner turned auto-open off), AND artist_acceptance — a direct-booking org has no
  // offer step, and open-offer-tier now 409s in direct mode (Task 8), so the acceptance
  // half avoids pointless failing invokes. Only the auto-open is gated; the date sync
  // above always runs.
  let tiersOpened = 0;
