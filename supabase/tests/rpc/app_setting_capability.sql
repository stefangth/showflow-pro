...
-- from `edit_hire_order_settings`. Unmapped here they would fall into the
-- admin-only arm and a capability-enabled producer's Save would fail on RLS.
SELECT is(public.app_setting_capability('hire_order_copy'), 'producer_can_edit_hire_order_settings', 'pdf copy key -> edit_hire_order_settings');
SELECT is(public.app_setting_capability('hire_order_theme'), 'producer_can_edit_hire_order_settings', 'pdf theme key -> edit_hire_order_settings');
