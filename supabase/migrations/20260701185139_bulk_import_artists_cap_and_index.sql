-- Spec B follow-up (review): server-side row cap (defense-in-depth; the client also
-- enforces MAX_IMPORT_ROWS) + a functional index backing the per-row dedup lookup.
create index if not exists artists_org_lower_email_idx
  on public.artists (org_id, lower(email))
