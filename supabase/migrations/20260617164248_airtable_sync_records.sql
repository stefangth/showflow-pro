...
-- ── Per-record history ───────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.airtable_sync_record_log (
...
CREATE INDEX IF NOT EXISTS idx_airtable_sync_record_log_sync_log ON public.airtable_sync_record_log (sync_log_id);
CREATE INDEX IF NOT EXISTS idx_airtable_sync_record_log_org_created ON public.airtable_sync_record_log (org_id, created_at);
CREATE INDEX IF NOT EXISTS idx_airtable_sync_record_log_org_record ON public.airtable_sync_record_log (org_id, airtable_record_id);
...
-- RLS: org isolation (restrictive) + admin read (mirrors airtable_sync_log).
ALTER TABLE public.airtable_sync_record_log ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS org_isolation ON public.airtable_sync_record_log;
CREATE POLICY org_isolation ON public.airtable_sync_record_log AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.is_org_member(auth.uid(), org_id))
  WITH CHECK (public.is_org_member(auth.uid(), org_id));

DROP POLICY IF EXISTS "Admins can view sync record logs" ON public.airtable_sync_record_log;
...
-- No INSERT policy: rows are written only by the airtable-poll edge function using the
-- service-role client, which bypasses RLS. (Avoids a WITH CHECK (true) audit-table policy.)
