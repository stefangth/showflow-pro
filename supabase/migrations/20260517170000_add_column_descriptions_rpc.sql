...
-- show_dates
COMMENT ON COLUMN show_dates.id IS 'ID';
...
COMMENT ON COLUMN show_dates.city_id IS 'City';
COMMENT ON COLUMN show_dates.status IS 'Date status';
...
-- shows
COMMENT ON COLUMN shows.id IS 'ID';
...
-- cities
COMMENT ON COLUMN cities.id IS 'ID';
...
COMMENT ON COLUMN artists.status IS 'Artist status';
...
AS $$
  ...
  WHERE n.nspname = 'public'
    AND c.relkind = 'r'
    AND c.relname = ANY(ARRAY['show_dates', 'shows', 'cities', 'bookings', 'artists']);
    -- Keep in sync with TABLE_COLUMNS in src/features/editor/columnRegistries.ts
$$;

REVOKE EXECUTE ON FUNCTION get_column_descriptions() FROM PUBLIC;
