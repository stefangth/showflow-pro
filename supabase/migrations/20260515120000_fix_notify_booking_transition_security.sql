-- notify_booking_transition writes to booking_audit_log and notifications,
-- both of which restrict INSERT to admins/producers. As SECURITY INVOKER the
-- trigger inherits the caller's role — when an artist accepts an offer the
-- INSERTs fail RLS and the whole booking UPDATE rolls back. Re-create the
-- function as SECURITY DEFINER so it runs with the function-owner's
-- privileges (RLS bypassed for system-managed audit/notification writes).
