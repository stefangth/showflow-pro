-- Demo mode: redefine seed_demo_org to seed plain CITY names in public.cities.
--
-- The original seed (20260816205135_demo_mode_seed_rpcs.sql) inserted
-- theatre/venue names ('Rheinbühne Köln', 'Stadttheater Bonn', ...) into the
-- cities table, each embedding a real city. The cities table holds plain city
-- names; venue names belong on show_dates.venue (which already uses its own
-- independent literal array). This migration replaces the city-name array with
-- the plain city names ['Köln','Bonn','Düsseldorf','Aachen']. Nothing else
-- changes: cities are linked to show_dates by array index/id, never by literal
-- name string, so the display names are safe to change.
--
-- Existing migrations must never be edited (version drift aborts deploys), so
-- this is a forward CREATE OR REPLACE that reproduces the current function body
-- verbatim except for the one array. CREATE OR REPLACE preserves the existing
-- grants; they are re-issued below to keep the migration self-contained.
...
begin
  ...
  -- Cities (4 for a multi-venue feel). Plain city names; venue names live on
  -- show_dates.venue below.
  for i in 1..4 loop
    insert into public.cities (org_id, name)
    values (p_org, (array['Köln','Bonn','Düsseldorf','Aachen'])[i])
...
-- Service-role-only (mirrors the original migration): block direct
-- anon/authenticated calls and grant service_role explicitly. CREATE OR REPLACE
-- already preserves grants; these are re-issued to keep this migration
