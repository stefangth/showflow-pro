...
| `claude/adoring-knuth-xkliev` | Current Claude Code session (scheduled docs-sync task) |
...
| Issue | Impact | Notes |
|-------|--------|-------|
| No prod seed-test-data edge function | Dev setup friction (production/staging only) | Must bootstrap first org admin out-of-band (Supabase dashboard); org admin then invites others via Settings → People (folded in from the old `/admin` page during the Get running cutover, 2026-08-25 — `/admin` now just redirects to `/settings?tab=people`). **Local dev is no longer affected** — since 2026-08-08 the local Supabase stack (`npm run local:up`) auto-applies `supabase/seed.sql`, seeding logins `admin@`/`producer@`/`artist@example.com` (password `showflow-dev`); see `docs/runbooks/local-development-stack.md`. |
| Releases since `v1.9.0` are untagged | Process gap | `package.json`/changelog are at `1.17.2` (Aug 25). Tagging *was* caught up once already — `v1.4.1`–`v1.9.0` were all cut in one batch on 2026-07-15 — but versions `1.9.1`–`1.17.2` have shipped since with no corresponding `git tag`. Catch up again when convenient. |
...
| 2026-08-25 | The scattered onboarding UI (dashboard first-run welcome panel + sample-data preview, the booking-flow and hire-orders per-module setup rails, `SetupChecklistSheet` shared chrome) was replaced wholesale by a single `/get-running` board ("Wireflow v3"), now the unconditional default for every org; `/admin` was folded into `/settings?tab=people` and Help moved into the sidebar's system nav cluster | One ordered, role-scoped checklist beats three inconsistent rails plus a page nobody used for account management alone; the old step editors (`FlowStep`, `SlotsStep`, `TimingStep`, `LetterheadStep`, `TermsStep`, `CountersignStep`, `EligibilityStep`/`LadderStep` — now `panels/{Eligibility,Ladder}PanelBody`) were kept and reused as task-panel bodies rather than rewritten. See `docs/superpowers/plans/2026-08-25-wireflow-v3-phase-5-v1-board-cutover.md` and the earlier phase plans in the same directory |
| 2026-08-24 | Renamed the Tailwind radius scale to collision-proof semantic keys (`chip`/`field`/`control`/`card`/`icon`/`pill`) and added a build-time guard (`scripts/tailwindThemeCollisions.test.ts`) after a real production bug: the old single-letter keys (`l`, `s`) silently collided with Tailwind's own corner/side utility namespace, so cards rendered 4px on the left and 10px on the right | A key like `l` (radius) and `l` (Tailwind's `rounded-l`) both compile — Tailwind wins the cascade on shared corners, so the bug was invisible until pixel-diffed. Semantic names can't collide with Tailwind's built-in single/double-letter corner suffixes |
| 2026-08-24 | Added the `Token` primitive (Geist Mono, restricted to machine-readable values: ids, reference numbers, keys, scopes, function names, status codes, versions) alongside `Metric` (Geist Sans, tabular figures, for anything read aloud as a quantity/date/time); banned raw `font-mono` in feature code via lint | Mono had crept onto ordinary numbers (dates, counts) purely for a "technical" look, which reads as machine data to users. Splitting the two token types by what a human would read the value as (a quantity vs. an identifier) fixed the visual inconsistency and gave lint something to enforce |
...
| Date | Change |
|------|--------|
| 2026-09-07 | Updated `CLAUDE.md` and `memory.md` — a scheduled audit found 71 commits and four releases (`1.15.1`→`1.17.2`) of drift since the 2026-08-10 sync, the largest single item being a full onboarding rewrite the architecture tree didn't mention at all. Fixed: the `src/components/` tree (deleted `dashboard/firstRun/`, trimmed `bookings/setup/` and `hireOrders/setup/` down to their surviving step editors, replaced `setup/`'s description now that only `useRailDismissed` is left, added a new `getRunning/` entry describing the Wireflow v3 board + its `v3/`, `panels/`, `board/`, `steps/` subtrees); `src/lib/` (added `getRunning/` — the pure task-composer model); `src/hooks/` (added `useGetRunning`/`useGetRunningV3`/`useGetRunningNavVisible`/`useHireOrderExtraSetup`, reframed `useBookingSetup`/`useHireOrderSetup` as consumed-by-the-board rather than rail-readiness); `src/pages/` (removed `AdminPage` — deleted, `/admin` is now a bare redirect — added `GetRunningPage`, fixed `/hire-orders` → `/contracts` path references left over from the route rename, fixed `ROUTES.DASHBOARD`/`ROUTES.BOOKINGS` raw-path mentions to `/today`/`/dates`); the New-page-checklist's stale "pinned below the setup rail" line; the i18n Phase-2 note's stale "shared with the bookings and hire-orders setup rails" line; the versioning section's tag range (`1.9.1`–`1.17.2`, verified against the unshallowed remote that no new tags exist past `v1.9.0`). Did not add: an ADR for the Get Running rewrite (it has its own paper trail in `docs/superpowers/plans/` and `specs/`; writing one after the fact wasn't this session's call to make) or an entry in `docs/system-map.md` (the rewrite is pure frontend UI, no DB trigger/automation involved). Current app version: `1.17.2` |
| 2026-08-24–25 | Radius scale renamed to collision-proof semantic keys (`chip`/`field`/`control`/`card`/`icon`/`pill`) after single-letter keys silently collided with Tailwind's own corner-utility namespace (`scripts/tailwindThemeCollisions.test.ts` now guards it); added the `Token` primitive (mono, machine-readable values only) alongside `Metric` (tabular sans, human-read quantities) and banned raw `font-mono` in feature code; offers-cockpit redesign — tier ladder collapsed into one state-coded round strip, dry-run error/loading states, Airtable+Sheets import added to the Dates toolbar; "Production"/"casting breakdown"/"parts" terminology and "Sources" (Airtable+Sheets) naming rolled out; v1.17.2 "A steadier setup walkthrough…" ships |
| 2026-08-17–25 | **Get running board / "Wireflow v3"** — replaced the dashboard first-run welcome panel, the booking-flow and hire-orders per-module setup rails, and the shared `SetupChecklistSheet` chrome with one `/get-running` board (11 tasks / 3 phases, role-scoped, right-side task panel reusing the existing step editors). Folded `/admin` into `/settings?tab=people`; moved Help into the sidebar's system nav cluster. Shipped initially behind a runtime override (`getrunning_v3_enabled`) with the old v1 board as fallback, then the 2026-08-25 cutover (#353) made v3 the unconditional default for every org and deleted the v1 board, its dev harness, and the toggle entirely. See `docs/superpowers/plans/2026-08-17-get-running-board.md` and the `2026-08-23-wireflow-v3-phase-*` plans |
| 2026-08-21 | v1.17.1 — Settings now opens on "How this org works" instead of Organization; a steadier, more consistent visual pass across cards/badges/labels/buttons; the sidebar Dates badge count now matches the "Needs you" list; several wording fixes distinguishing "waiting on you" from "waiting on the production team" |
| 2026-09-01 | Bounded `airtable_sync_record_log` growth with a `created_at` index for pruning |
| 2026-08-24 | Dependency bumps: `recharts` 3.10.1 (+ `chart.tsx` prop-type migration), `zod` 3.25.76 → 4.4.3, dev-tooling group |
| 2026-08-15 | v1.17.0 "Emails and contracts in your workspace's language" — German transactional emails and hire-order PDFs when the workspace language is German; admin-set workspace language (Settings → Organization); bilingual email-template preview |
| 2026-08-14 | v1.16.0 "Your language, and a Help center that speaks it" — EN/DE language switcher (account menu), a role-aware Help center (`HelpPage`, `src/lib/help/`), page-mini four-step guides shipped app-wide (`src/lib/minis/`, `src/components/minis/`) |
| 2026-08-12 | v1.15.1 — longer-lived invitation links; choice of magic-link or password sign-in after accepting an invite, password also settable later from Profile |
