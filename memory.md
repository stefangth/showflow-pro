...
| Releases since `v1.9.0` are untagged | Process gap | `package.json`/changelog are at `1.18.0` (Sep 15). Tagging *was* caught up once already — `v1.4.1`–`v1.9.0` were all cut in one batch on 2026-07-15 — but versions `1.9.1`–`1.18.0` have shipped since with no corresponding `git tag`. Catch up again when convenient. |
...
| Date | Change |
|------|--------|
| 2026-09-15 | Workspace type (`org_kind`) feature complete across PR 1 to PR 3 (#373 plus the PR 3 branch `claude/pr-3-plan-k27wnp`). An org is Live production or Staffing agency (Settings, Organization); the whole UI, transactional emails, and hire-order PDFs render that workspace's vocabulary via `src/lib/orgKind.ts` + `VocabularyBridge`. PR 3 shipped the client-presentation and server halves: kind-aware `roleDescription` (accept-invite + People pane), cockpit cast-group titles, the calendar standby pill and SlotsStep rail + stored slot names, page-mini illustration nouns (`ArtEntry`/`resolveArt`), a workspace-type help explainer (`A3.18`), and `resolveEmailCopy`/`resolveHireOrderCopy` vocabulary threading through `send-transactional-email` and `generate-hire-orders`. Standby is kept for both types (reworded, not hidden); the orphaned dashboard `SAMPLE_PREVIEW` fixture was deleted. Production orgs render byte-identically except an owner-approved "hire order" to "contract" wording alignment. Current app version: `1.18.0`. |
