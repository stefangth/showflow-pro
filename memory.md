...
| `claude/adoring-knuth-3bflhw` | Current Claude Code session (scheduled docs-sync task) |
...
- **Entitlements** ("does this module exist for this org") — `src/lib/entitlements.ts` `FEATURE_REGISTRY`, now three keys: `booking_flow`, `hire_orders`, and `language_packages` (gates the language switcher + German UI; added since the last sync). Toggled per org from Platform → Organizations → Edit org → Modules; `hire_orders` and `language_packages` both ship dark (default off).
- **Capabilities** ("who may act within an enabled module") — `src/lib/capabilities.ts` `CAPABILITY_DEFS`, now 29 producer capabilities (was 27; `manage_skills` and `edit_email_templates` were added since), resolved lock → org override → platform default → registry default.
...
| `notify-cast` and `import-sheet-dates` edge functions are missing from `docs/system-map.md` / `src/data/systemMap.ts` | Doc gap | Both functions shipped in the Get Running v3 / Autopilot Today work (see below) but were never added to the automation system map, even though CLAUDE.md requires updating both "in the same PR as any automation change." Every other edge function added around the same time (`exchange-invitation`, `org-purge-removed-user`) IS documented there. Needs a follow-up PR that reads the existing trigger→function→data→effect table format carefully before adding these two rows. |
...
| 2026-08 (mid-month, exact commit predates this session's shallow clone window) | Dashboard rebuilt into two surfaces: `/today` ("Autopilot Today" — `src/components/today/`, a daily action board for admin/producer) and `/get-running` (the guided setup board, `src/components/getRunning/`). The old `dashboard/firstRun/` welcome-panel-plus-setup-rail concept was deleted outright, not renamed. Sibling URL relabels landed alongside it: `ROUTES.BOOKINGS` → `/dates`, `ROUTES.HIRE_ORDERS`(+detail/edit/template) → `/contracts...`. All old paths (`/dashboard`, `/bookings`, `/hire-orders...`) still resolve via `src/features/auth/legacyRedirects.tsx`, kept indefinitely for bookmarks/emails/the marketing site | Splits "what needs a human today" (Today) from "how do I finish setting this org up" (Get Running) instead of cramming both into one dashboard; URL relabels align paths with the renamed nav/sidebar copy ("Today", "Dates", "Contracts") without touching any code/DB identifiers |
| 2026-08 (within the "v3 cutover" commits, ~Aug 20-25) | Get Running's v1 board was deleted; the v3 board (`src/components/getRunning/v3/`, `useGetRunningV3`) is now the only board, unconditionally shown to every org (no runtime toggle) | The v3 rewrite (step-by-step wizard, one title+action per step, narrower-window layout) had been running behind a flag; once validated it fully replaced v1 rather than staying dual-run, removing a maintenance burden and a source of "which board am I looking at" confusion |
| 2026-08 (same window) | `language_packages` entitlement added to `FEATURE_REGISTRY` (ships dark); `manage_skills` and `edit_email_templates` producer capabilities added to `CAPABILITY_DEFS` (27 → 29); i18n namespaces grew from 2 (`common`, `help`) to 30, covering nearly every UI domain | Tracks the German-language rollout and the "create a skill inline while editing a casting breakdown" + email-template-editor features reaching general availability behind their own gates |
...
| Date | Change |
|------|--------|
| 2026-08-31 | Updated `CLAUDE.md`, `memory.md`, and `docs/adr/README.md` — a scheduled audit found ~70 commits and three releases (`1.15.1`→`1.17.2`) of drift since the 2026-08-10 sync, headlined by an undocumented rebuild: the dashboard split into `/today` ("Autopilot Today") and `/get-running` (the guided setup board, v3-only after the v1 board was deleted), plus a URL relabel (`/dashboard`→`/today`, `/bookings`→`/dates`, `/hire-orders...`→`/contracts...`, all old paths kept alive via `src/features/auth/legacyRedirects.tsx`). Fixed in `CLAUDE.md`: the `src/components/` architecture tree (rewrote `dashboard/`, added `getRunning/`, `today/`, `help/`, `skills/`, updated `catalog/`'s casting-breakdown note); `src/data/` domain list (+8: `autopilot`, `castProductionFees`, `datesSource`, `orgAdmins`, `sheetImport`, `slots`, `tierLadder`, `trustStats`); `src/hooks/` list (+14); `src/lib/` list (+`autopilot/`, `sheetImport/`, `getRunning/`); `src/pages/` Key pages list (added `GetRunningPage`, `HelpPage`, fixed the hire-order route paths, called out the URL relabel + `legacyRedirects.tsx`); Edge functions section (added `exchange-invitation`, `import-sheet-dates`, `notify-cast`, `org-purge-removed-user`); a new Naming-section rule on URL paths being copy, not identifiers, and one on the production/casting-breakdown/parts UI vocabulary; the i18n section's namespace list (2 → 30 namespaces, "Phase 2" language retired since it's no longer accurate); the versioning tag-range line and capability count (27 → 29, also fixed in `docs/adr/README.md`). Flagged but deliberately NOT fixed myself (needs dedicated follow-up, format risk too high for a docs-sync pass): `notify-cast`/`import-sheet-dates` missing from `docs/system-map.md`/`src/data/systemMap.ts` despite CLAUDE.md's own same-PR rule — logged in Known issues below. Confirmed NOT stale (checked, left alone): the page-mini registry (Get Running deliberately has no mini, per its own design doc), `docs/superpowers/specs/*` paths CLAUDE.md cites, and the entitlements/capabilities mirror files. Current app version: `1.17.2` |
