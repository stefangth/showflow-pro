...
| `claude/adoring-knuth-ny7laa` | Current Claude Code session (scheduled docs-sync task) |
...
| Releases since `v1.9.0` are untagged | Process gap | `package.json`/changelog are at `1.17.3` (Sep 11). Tagging *was* caught up once already — `v1.4.1`–`v1.9.0` were all cut in one batch on 2026-07-15 — but versions `1.9.1`–`1.17.3` have shipped since with no corresponding `git tag`. Catch up again when convenient. |
| Get Running has two parallel "is setup done" models | Correctness risk, not yet observed to misbehave | `src/lib/getRunning/tasks.ts` is the older v1 11-task composer, still used by `HomeLanding` (the one-time post-login redirect: Dashboard vs. `/get-running`) and `AcceptInvitePage`. `src/lib/getRunning/steps.ts` is the v3 16-step composer that actually drives the rendered board and the sidebar nav badge (`useGetRunningNavVisible`). v3 merged v1's separate `ladder` + `eligibility` steps into one `coverage` step, so the two models' "done" answer can diverge for an org sitting exactly on that boundary. Found during the 2026-09-14 doc sync; worth reconciling onto one model. |
| Dead code from the retired dashboard-first-run feature | Minor — no functional impact, just clutter | `src/hooks/useFirstRunMetrics.ts` and `src/lib/dashboard/firstRun.ts` are unused outside their own test files (the feature they measured was replaced by Get Running v3 / Autopilot Today). Not deleted as part of this doc-only sync — flagging for a future cleanup PR to confirm and remove. |
...
| 2026-08-24–09-01 | "Get Running v3" became the single unified onboarding surface (route `/get-running`, admin/producer-only), replacing the fragmented dashboard-first-run rail, the booking-flow setup rail, and the hire-orders setup rail | One 16-step, 3-phase wizard (`get_dates`/`bookable`/`paperwork`) is easier to reason about and localize than three independently-gated rail components; step bodies from the old rails (`FlowStep`, `TimingStep`, `LetterheadStep`, `TermsStep`, `CountersignStep`, …) were kept and re-wired in as wizard steps rather than rewritten. `/dashboard` no longer does first-run branching itself — it always renders Autopilot Today (`src/components/today/`) for admins/producers and `ArtistDashboard` for artist-only users; the one-time "send a mid-setup org to Get Running instead" decision moved to `src/features/auth/HomeLanding.tsx` at the app root (`/`). See the 2026-09-14 recent-changes row and the "Get Running has two parallel is-setup-done models" known issue above |
...
| Date | Change |
|------|--------|
| 2026-09-14 | Updated `CLAUDE.md` and `memory.md` — a scheduled audit found the Get Running v3 cutover (landed 2026-08-24–09-01, after CLAUDE.md's last edit) had left CLAUDE.md's architecture tree pointing at deleted code: the whole `src/components/dashboard/firstRun/` directory, most of `src/components/setup/`, and the "setup rail" halves of `src/components/bookings/setup/` and `src/components/hireOrders/setup/` no longer exist. Rewrote those tree entries plus added the new `today/` (Autopilot Today board) and `getRunning/` (the v3 wizard) component dirs, `HelpPage`/`GetRunningPage` to the pages list, ~15 new hooks and ~8 new `src/data/` domains, and corrected two stale route claims along the way (`ROUTES.DASHBOARD` is `/today`, not `/dashboard`; the Get Running mini sits at the header→body seam, not "below the setup rail"). Also noted `expire-offers`' new retry-on-timeout behavior and `cron-health-watcher`'s new two-strikes alert debounce (both shipped 2026-09-11). Flagged but did not touch: two parallel "is setup done" models (`getRunning/tasks.ts` v1 vs `steps.ts` v3) that can disagree, and two small orphaned files from the retired dashboard-first-run feature (`useFirstRunMetrics.ts`, `lib/dashboard/firstRun.ts`) — both recorded as known issues rather than deleted, since actual code deletion is out of scope for a docs sync. Current app version: `1.17.3`; releases since `v1.9.0` remain untagged (now `1.9.1`-`1.17.3`) |
| 2026-09-01–11 | Internal reliability fixes (no changelog entry — ops/infra, not user-facing): bounded `airtable_sync_record_log` growth (it had grown past 1GB in production from re-snapshotting the full Airtable payload on every recurring held/updated row; now only `imported_new`/`error` keep `raw_fields`) (#360); debounced `cron-health-watcher` HTTP-failure alerts to two consecutive failures so a single self-healing 500/503 doesn't page super-admins (#363) |
| 2026-08-25–09-11 | v1.17.1–v1.17.3 — a steadier Get Running v3 walkthrough (per-step title + footer action, narrow-window layout, "Read more" deep-links into Help, finished steps read as finished everywhere), skills/cities addable inline from the production casting-breakdown step, Airtable/Google-Sheet dates import from the New date button, "Production"/"casting breakdown"/"parts" terminology pass, and (1.17.3) an hourly-expiry reliability fix for a transient Supabase gateway timeout |
| 2026-08-15–24 | v1.16.0–v1.17.0 — account-menu language switcher (English/German, `language_packages` entitlement, dark by default), a role-aware Help center (`src/pages/HelpPage.tsx`, `src/lib/help/`), the "page mini" four-step page guide rolled out app-wide, German transactional emails + hire-order PDFs; the Wireflow "Get Running" v3 board cutover (#349–#353) retired the v1 get-running board, the dashboard-first-run rail, and the booking-flow/hire-orders setup rails in favor of the single v3 wizard, with `/dashboard` now always showing the new Autopilot Today board (`src/components/today/`) to admins/producers |
| 2026-08-12–14 | v1.15.1 — longer-lived invitation/magic links, password sign-in as an alternative to magic links (added/changed from Profile) |
